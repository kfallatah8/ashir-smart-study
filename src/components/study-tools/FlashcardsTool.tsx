...
const FlashcardsTool = () => {
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      await generateTool('flashcards');
      toast({
        title: t('Success'),
        description: t('Your flashcards are being generated'),
      });
    } catch (error) {
        ...
        <CardContent>
          <Tabs defaultValue="create" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="create">{t('Create Flashcards')}</TabsTrigger>
              <TabsTrigger value="saved">{t('Saved Flashcards')}</TabsTrigger>
            </TabsList>
